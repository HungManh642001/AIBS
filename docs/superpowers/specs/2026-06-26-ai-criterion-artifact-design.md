...
- **RAG / segmentation (Qdrant, PRD §6–7):** loại bỏ cho bài toán này. Vì HSDT đã tách file theo loại, định tuyến bằng metadata (`artifact_type`) vừa chính xác vừa nhẹ hơn hẳn. Với HSMT lớn (100+ trang), ta dùng **định vị mục theo cấu trúc chuẩn** (mục §4.6) — nhẹ hơn nhiều so với RAG. RAG chỉ cần khi một file chứa nhiều loại, HSMT phi chuẩn cực lớn, hoặc tài liệu quá lớn — để dành nâng cấp tương lai, không nằm trong phạm vi này.
...
```
   ...
   │ 1a. Định vị mục TCĐG+BDS │              │ User chọn loại + AI kiểm  │
   │ 1b. AI: tiêu chí →       │              │ tra khớp (cảnh báo nhầm)  │
   │   required_artifacts +   │              │ → mỗi file có artifact_type│
   │   sub_checks[] + ngưỡng  │              └────────────┬─────────────┘
   │   (giải tham chiếu chéo) │                            │
   └───────────┬─────────────┘                            │
...
**`thong_so` (JSON) ví dụ:** `{"nguong": "3% giá gói thầu", "gia_tri_so": 150000000, "don_vi": "VND", "so_ngay": 120, "nguon": "Bảng dữ liệu đấu thầu, tr.7", "can_review": false}` — AI trích ở bước đề cương; chuyên gia sửa được.
  - `nguon`: vị trí (mục/trang HSMT) nơi lấy con số — phục vụ truy xuất nguồn gốc, đặc biệt khi con số nằm ở mục khác với phần Tiêu chuẩn đánh giá (xem §4.7).
  - `can_review: true` khi AI **không giải được** tham chiếu (số mơ hồ/vắng) → chuyên gia phải điền ở màn Đề cương trước khi chạy.
...
### 4.1 `extract_de_cuong(sections) -> list[criterion]`
Thay cho `extract_criteria`. **Đầu vào không phải toàn bộ HSMT** mà là các mục đã định vị (§4.6): phần *Tiêu chuẩn đánh giá (TCĐG)* + *Bảng dữ liệu đấu thầu (BDS)*. AI làm cả bước **giải tham chiếu chéo** (§4.7) để điền con số. Mỗi tiêu chí:
```json
{ "nhom": "hop_le", "ten": "Bảo đảm dự thầu",
  ...
  "sub_checks": [
    ...
    {"ten": "Giá trị ≥ ngưỡng HSMT", "check_type": "value_threshold",
     "thong_so": {"gia_tri_so": 150000000, "don_vi": "VND", "nguon": "Bảng dữ liệu đấu thầu, tr.7", "can_review": false}, "required_artifact": "bao_dam_du_thau", "blocking": true},
    {"ten": "Thời hạn hiệu lực ≥ yêu cầu", "check_type": "date_validity",
     "thong_so": {"so_ngay": 120, "nguon": "Bảng dữ liệu đấu thầu, tr.7", "can_review": false}, "required_artifact": "bao_dam_du_thau", "blocking": true}
  ],
  "proposed_artifacts": [] }
```
...
### 4.6 Định vị mục HSMT — xử lý tài liệu lớn (`locate_hsmt_sections`)
HSMT thực tế 100+ trang, **không nhét cả tập vào AI** (vượt token budget và không cần). HSMT theo mẫu Bộ KH&ĐT có cấu trúc chuẩn; phần liên quan tới bóc tách chỉ nằm ở vài mục:
- **Tiêu chuẩn đánh giá (TCĐG)** — chứa *quy định* đánh giá (thường Chương III).
- **Bảng dữ liệu đấu thầu (BDS/BDL)** — chứa *con số cụ thể* (giá trị bảo đảm dự thầu, số ngày hiệu lực, ngày đóng thầu…) (thường Chương I/II).

`locate_hsmt_sections(hsmt_pages) -> {tcdg: [pages], bds: [pages]}`:
- Heuristic: dò mục lục + heading + từ khóa ("Tiêu chuẩn đánh giá", "Bảng dữ liệu đấu thầu", "Bảo đảm dự thầu"…) để tìm dải trang của mỗi mục; có thể nhờ AI xác nhận ranh giới khi heading không rõ.
- Chỉ các trang định vị được (thường ~5–15 trang) đưa vào `extract_de_cuong`.
- **Fallback**: HSMT phi chuẩn / không có heading rõ → chunk map-reduce (chia nhỏ, bóc từng phần, gộp + khử trùng) — chấp nhận tốn token hơn.

### 4.7 Giải tham chiếu chéo (cross-reference resolution)
Phần TCĐG thường **chỉ tham chiếu** chứ không nêu số: vd *"thư bảo lãnh có giá trị và hiệu lực theo yêu cầu của HSMT"* — con số nằm ở **Bảng dữ liệu đấu thầu** (mục khác).
```
TCĐG:  "thư bảo lãnh có giá trị và hiệu lực theo yêu cầu HSMT"
                      │  AI tra chéo sang ▼
BDS:   "Giá trị bảo đảm dự thầu: 150.000.000đ; Hiệu lực: 120 ngày"
                      ▼
sub_check value_threshold → thong_so: {gia_tri_so: 150000000, don_vi: VND,
                                       nguon: "BDS, tr.7", can_review: false}
```
Vì vậy `extract_de_cuong` **luôn nhận cả TCĐG + BDS** làm context. Quy tắc:
- Giải được → điền số + `nguon` (vị trí lấy số), `can_review=false`.
- Không giải được (số mơ hồ/vắng/nhiều phương án) → để trống số, `can_review=true` → chuyên gia điền ở màn Đề cương (khớp hàng "Thiếu ngưỡng số/ngày" §5.2).
...
- `services/hsmt_locator.py` — `locate_hsmt_sections` (định vị TCĐG + BDS; fallback chunk map-reduce)
...
- `locate_hsmt_sections`: định vị đúng dải trang TCĐG + BDS trên HSMT mẫu nhiều trang; fallback khi không có heading
- `extract_de_cuong`: tiêu chí có `required_artifacts` + `sub_checks` đúng shape; **giải tham chiếu chéo** điền `thong_so.nguon`; đặt `can_review=true` khi không giải được số
...
5. **HSMT 100+ trang**: chỉ các mục TCĐG + BDS được đưa vào AI (không nhét cả tập); tham chiếu chéo ("theo yêu cầu HSMT") được giải về con số kèm `nguon`, hoặc đánh dấu `can_review` cho chuyên gia.
6. Demo chạy được với mock (không GPU); cấu trúc sẵn sàng cho Qwen3 thật.
7. Pattern tổng quát, 3 nhóm còn lại có thể áp dụng lại mà không đổi kiến trúc.

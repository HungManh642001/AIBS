...
SYS_EVAL = (
    ...
    "THỨ BẬC NGUỒN CHUẨN (quan trọng nhất): YÊU CẦU GỐC là nguyên văn HSMT — nó định PHẠM VI và là "
    "TRẦN về mức nghiêm ngặt. CHUẨN là giá trị đã tra được từ HSMT kèm điều khoản nguồn — CHUẨN cụ "
    "thể hơn YÊU CẦU GỐC là HỢP LỆ, vẫn dùng để so. YÊU CẦU chỉ là DIỄN GIẢI của YÊU CẦU GỐC:\n"
    "- HSDT thỏa YÊU CẦU -> kết luận 'đạt' bình thường.\n"
    "- HSDT KHÔNG thỏa một điều kiện CHỈ có trong YÊU CẦU mà KHÔNG truy được về YÊU CẦU GỐC hoặc "
    "CHUẨN -> TUYỆT ĐỐI KHÔNG vì điều kiện đó mà trả 'không đạt'/'cần làm rõ'; hãy xét tiếp theo "
    "YÊU CẦU GỐC, và BẮT BUỘC ghi_chu nêu rõ đã bỏ qua điều kiện nào (để chuyên gia rà lại).\n"
    "- Chỉ kết luận trong phạm vi NỘI DUNG KIỂM TRA này; điều kiện thuộc nội dung khác của tiêu "
    "chí -> bỏ qua, đừng kết luận thay.\n"
    "- YÊU CẦU nêu nhiều phương án nối bằng 'hoặc'/'HOẶC' -> chỉ cần HSDT thỏa MỘT phương án là "
    "'đạt'. TUYỆT ĐỐI KHÔNG đòi thỏa hết; bang_chung trích phương án đã thỏa, ghi_chu nói rõ dùng "
    "phương án nào. (Vd đơn liên danh: từng thành viên ký, HOẶC thành viên đứng đầu ký thay mặt "
    "theo phân công — nhà thầu làm cách nào cũng hợp lệ.)\n"
    "- CHUẨN ghi '(không có)' mà YÊU CẦU GỐC (hoặc CHUẨN) trỏ tới giá trị/chuẩn trong HSMT -> KHÔNG "
    "kết luận đạt/không đạt, trả 'cần làm rõ' (thiếu căn cứ chuẩn).\n"
...
def eval_prompt(nd_item: dict[str, Any], hsdt_text: str, cross: bool = False, *,
                vendor_ctx: Any | None = None, profile: Any | None = None,
                yeu_cau_goc: str = "", anh_em: list[str] | None = None) -> str:
    ...
    yeu_cau_goc: nguyên văn HSMT cấp tiêu chí — đối chứng chống lạm phát yêu cầu (bước STRUCT của
    decompose KHÔNG thấy HSMT, nên điều kiện trong `yeu_cau` không truy được về gốc là bịa).
    anh_em: tên các nội dung kiểm tra KHÁC của cùng tiêu chí (1 gốc -> N yêu cầu) — chống trôi
    phạm vi sang kết luận thay phần của lượt chấm khác.
    """
    ...
    goc = f"YÊU CẦU GỐC (nguyên văn HSMT, cấp tiêu chí): {yeu_cau_goc}\n" if yeu_cau_goc else ""
    khac = (f"CÁC NỘI DUNG KHÁC của tiêu chí (lượt chấm khác phụ trách — ĐỪNG kết luận thay): "
            f"{'; '.join(anh_em)}\n" if anh_em else "")
    return (
        ...
        f"{khac}"
        f"{goc}"
        f"YÊU CẦU (diễn giải từ yêu cầu gốc — tham khảo): {nd_item.get('yeu_cau', '')}\n"

...
def _ce(ten="Bảo đảm dự thầu", ket_qua=KET_QUA_KHONG, verdicts=None,
        nhom="hop_le"):
    return CriterionEval(nhom=nhom, ten=ten, ket_qua=ket_qua,
...
def test_markdown_can_xu_ly_lists_only_actionable():
    ...
    md = to_markdown(_result(criteria=[
        _ce(), _ce(ten="Đạt tuốt", ket_qua=KET_QUA_DAT,
...
def test_markdown_na_section_shows_reason():
    md = to_markdown(_result(criteria=[_ce(
        ten="Thỏa thuận liên danh", ket_qua=KET_QUA_KHONG_AP_DUNG,
...
def test_markdown_na_criterion_not_in_details():
    md = to_markdown(_result(criteria=[
        _ce(), _ce(ten="Thỏa thuận liên danh", ket_qua=KET_QUA_KHONG_AP_DUNG,
...
def test_markdown_rule_verdict_shows_all_nguon_doc():
    md = to_markdown(_result(criteria=[_ce(
        ten="Đơn dự thầu", ket_qua=KET_QUA_DAT,
...
def test_to_markdown_does_not_mutate_criteria_order():
    r = _result(criteria=[_ce(ten="Đạt tuốt", ket_qua=KET_QUA_DAT,
                              verdicts=[_v(ket_qua=KET_QUA_DAT)]),
                          _ce(ten="Bị loại")])
...
def _ce_tt(ket_qua=KET_QUA_KHONG):
    """Kiểm tra thường trực là TIÊU CHÍ (nhóm phat_hien_bo_sung)."""
    from experiment.evaluate.schema import NHOM_PHAT_HIEN
    return _ce(ten="Người ký đơn dự thầu khớp đại diện pháp luật (ĐKKD)", ket_qua=ket_qua, verdicts=[_pv(ket_qua)], nhom=NHOM_PHAT_HIEN)
...
def test_kiem_tra_thuong_truc_vao_can_xu_ly_kem_nhan_nguon():
    ...
    assert "LOẠI" not in block                  # máy không kết luận loại
...
def test_kiem_tra_thuong_truc_vao_summary():
    """Đếm chung trong tổng kết như mọi tiêu chí."""
    r = _result(criteria=[_ce(ten="Đạt tuốt", ket_qua=KET_QUA_DAT,
                              verdicts=[_v(ket_qua=KET_QUA_DAT)]),
                          _ce_tt(KET_QUA_KHONG)])
    assert r.summary["n_tieu_chi"] == 2
